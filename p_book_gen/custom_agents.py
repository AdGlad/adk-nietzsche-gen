...
3. Draft a chapter of about {_chapter_length_range(chapter_number)[0]}–{_chapter_length_range(chapter_number)[1]} words with the following STRICT formatting rules:
   ...
   Heading and quote:
   - Begin the chapter with a SINGLE Markdown H2 heading in EXACTLY this format:
   ...
   Subheading and body:
   - After the quote, include a short subheading line (plain text, not a Markdown heading),
     for example: "How leaders respond when the ground shifts".
   - Then write the body text that aligns with the book topic, purpose, target audience,
     and the outline description.

   Additional formatting constraints:
   - Do NOT use any H1 headings (# ...) in this chapter. H1 is reserved for the book title.
   - Do NOT use any additional H2 headings. If you need internal structure, prefer paragraphs
     or, at most, modest emphasis, but avoid extra heading levels.
   - Do NOT insert any explicit page-break markup (for example <mbp:pagebreak /> or '---').
     Page breaks will be added later by another part of the workflow.
...
class ParseSpecAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        state["spec"] = spec
        for k in ["book_topic", "author_name", "author_bio", "author_voice_style", "target_audience", "book_purpose", "min_chapters"]:
            if k in spec:
                state[k] = spec.get(k)
